...
Plotting the first sequences

```{r}
# selecting the first sequence of every scenario, saving it in a tibble
df <- map(full_seqs[c(1, 101, 201, 301)], \(s) tibble(y = s$y, t = 1:length(s$y), pattern = s$pattern, mu = s$mu)) |> list_rbind()

# plotting the sequences with superimposed signals
ggplot(df) +
  geom_point(aes(x = t, y = y)) +
  geom_line(aes(x = t, y = mu), col = "red") +
  facet_grid(pattern~.) +
  theme_minimal()
```
...
Compare, in a simulation study, across the four different scenarios, performances of the various approaches:
...
evaluate_method <- function(y, mu_true, cps_true, method, penalty) {
  fit <- cpt.mean(y, penalty = penalty, method = method, Q = 100)

  # add the last point as a changepoint
  cp_est <- unique(c(cp_est, length(y)))
  cps_true <- c(cps_true, length(y))

  cp_est <- cpts(fit)  # integer vector of estimated CPs
  mu_hat <- rep(param.est(fit)$mean, times = diff(c(0, cp_est, length(y))))
